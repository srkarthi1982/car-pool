...
- 2026-05-03 (k): Final copy-only polish for the Car Pool landing page before freeze. Root cause: the product landing copy still had a few generic labels and less outcome-driven phrases after the product-content conversion. Refined `src/content/landing-demo.ts` only: expanded the Daily Flow panel to four concrete steps, changed hero-panel outputs to next-driver/fairness outcomes, replaced supporting items with outcome-based labels (`Next driver always known`, `Everyone sees the same record`, `No manual tracking needed`), tightened the fairness/reliability lead, replaced the showcase with a concrete Mon-Fri example week, and updated the final CTA to `Start your car pool in minutes`. Confirmed no public landing copy uses `system`, `template`, `standard`, or `reuse`. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03 (j): Implemented the real Car Pool product landing page using the existing Landing Page Standard V1 structure without layout/component changes. Root cause: the landing content still described the reusable landing system instead of the Car Pool product, leaving the public page feeling like documentation. Replaced all `src/content/landing-demo.ts` content with user-facing Car Pool messaging: fair rotation hero, trip tracking and accountability bullets, `Open Workspace` / `See how it works` CTAs, organized-group benefits, fair/reliable product pillars, 3-step “How it works,” weekly rotation showcase, and bottom CTA to start organizing the car pool. Confirmed landing content no longer mentions standard/system/template/reuse/future-app wording. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03 (i): Refined Landing Page Standard demo copy for external clarity and reuse strength without redesigning layout or component structure. Root cause: the visible demo still read partly like internal standard documentation, with version-heavy hero wording, engineering-style feature language, repeated pillar phrasing, vague showcase copy, and product-style CTAs. Updated `src/content/landing-demo.ts` only: softened hero title/subtitle into a reusable product system, changed CTAs to `View Example` / `Explore Structure`, added the subtle line `This is not a template — it is a system.`, simplified “What this standard adds,” varied pillar language, clarified optional showcase examples/outputs wording, and expanded future-app adoption copy to require real app-specific content. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03 (h): Replaced the trip history row `View` text button with the standard eye icon button. Root cause: the full trip history page still used a text `View` action while recent trips already used compact icon actions. Updated the row action to import/use `AvIcon` with `eye`, `app-icon-button`, and accessible `aria-label`/`title`; no behavior changes. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03 (g): Added V1-safe client-side pagination to the trip history page. Root cause: the history page could grow into a long unpaged list, especially after member/role filtering. Updated the full trip history route to request a larger bounded history batch (`limit: 500`), added Alpine pagination state with 10 trips per page, applied pagination after member/role filtering, reset the page when filters change, and added Previous/Next controls with a visible `Showing X-Y of Z` label. Did not change trip creation/edit, rotation, fairness, dashboard recent trips, or active traveller behavior. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03 (f): Made trip history member filtering role-aware after browser verification showed selecting a regular member could still show all records because every normal commute stores that member as a passenger. Root cause: the filter matched any participant row only, which was technically correct but not useful for answering “when did this member drive?” Updated history data to include participant role/status, added a role filter shown after selecting a member, defaulted member filtering to Driver, and kept Passenger, Absent, and Any role available for the broader history checks. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03 (e): Fixed trip history member filter select options not listing members. Root cause: the trip history page loaded `groupDetail` with members on the server but forgot to include it in the serialized `initialState`, so Alpine initialized `selectedGroupDetail.members` as an empty array and only the static `All members` option rendered. Added `groupDetail` to the page initial state. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03 (d): Added final V1 trip history member filter polish. Root cause: the full trip history page showed all trips without a passive way to answer when a member drove, rode, or was absent, and the existing history action only returned trip counts, not participant membership ids. Updated `listTripHistory` to include compact `participantMemberIds`, loaded group members into the trip history page state, added a separate `Filter by member` `AvSelect` with `All members` default and helper copy, filtered history rows client-side by driver/passenger/absent involvement, and added a no-match empty state. Did not change trip create/edit, rotation, fairness, or active traveller behavior. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03 (c): Fixed delete-group confirmation regression after adding the second member-removal confirm dialog. Root cause: `AvConfirmDialog` registers a single global `window.AvDialog.open`, and the new remove-member dialog overwrote the delete-group opener, so clicking Delete group could no-op before the server action ran. Updated the Alpine store to open confirm dialogs directly by target dialog id with native `showModal()`, preserving both delete-group and remove-member confirmation flows. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03 (b): Fixed working-day validation feedback for create/edit trip. Root cause: server-side working-day validation existed, but the Alpine submit path did not inspect `astro:actions` `result.error`, so invalid weekend/non-travel-day saves could look successful and close the drawer without a clear message. Updated create/update trip server validation to return `This date is outside the group’s selected travel days.`, parse date inputs at local midnight for weekday checks, added client-side selected-travel-day validation on trip date change and submit, and kept the drawer open by blocking close when create/update actions return errors. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-05-03: Added Astra-assigned V1 management fixes for group/member maintenance. Root cause: the group workspace had safe delete-group management but was missing owner/admin-only rename group, rename member display name, and safe remove-member flows. Added shared server-side group manager guard, protected `renameGroup`, `renameGroupMember`, and `removeGroupMember` actions with empty-name validation, group existence checks, owner/admin authorization, group-local member rename only, member removal confirmation UI, passenger/absent participant cleanup, stale assigned-driver clearing, and a hard block with `This member has driven trips. Reassign or delete those trips before removing them.` when the member has driven trips. Added existing drawer/dialog UI controls hidden from non-managers and refreshed group detail/trip history/traveller state after saves/removals. Verification: `npm run typecheck` ✅ (existing auth-page unused-import hints only), `npm run build` ✅.

- 2026-04-30 (c): Corrected active-traveller driver suggestion to keep global fairness as source of truth across all selected sets. Root cause: the dynamic active-traveller queue still had an equal-drive-count path that let the selected subset rotate locally, and the fairness normalization was calculated inside the selected set instead of the full group. Algorithm correction: selected travellers now only filter eligible candidates; ranking always uses global drive count first, then full-group-normalized fairness score, then missed/absence burden, then avoids immediate repeat only inside a true fairness tie, then stable rotation continuity/order/name. Upcoming preview now simulates the same global-fairness-first ranking by incrementing the suggested driver’s drive/fairness and treating other selected travellers as riders without mutating saved state. Updated rotation snapshot helper copy to state that selected travellers and full group history are both used. Verification: `npm run typecheck` ✅, `npm run build` ✅.

- 2026-04-30 (b): Added Astra-assigned dynamic active-traveller rotation fix for Car Pool V1. Root cause: dashboard driver suggestion and preview always used the full active roster, so temporary daily absences could not affect Today’s driver before logging a trip. Implemented non-persistent `travellingMemberIds` dashboard state defaulting to all active members, added “Today’s travelling members” checkbox chips above the rotation snapshot, blocked invalid fewer-than-2 selections, and changed Today’s driver/upcoming preview to use only the selected travelling set with the existing drive-count/fairness/rotation tie rules. New trip drawer now pre-fills driver from the selected-set suggestion, passengers from selected travellers excluding the driver, and absentees from unselected members; edit trip still loads strictly from saved trip data. No schema or membership persistence changes; delete group flow preserved. Verification: `npm run typecheck` ✅, `npm run build` ✅.

- 2026-04-30: Added Astra-assigned safe hard-delete group flow for Car Pool V1. Added protected `deleteGroup` server action with session validation, group existence check, owner/admin authorization, and explicit child cleanup order: trip participants for all group trips, trips, members, then group. Added group-detail danger-zone UI visible only to group owner or role-1 admin, using existing `AvConfirmDialog` with irreversible delete copy and redirecting back to `/app` after success. Schema audit found no separate fairness/dashboard/activity/audit tables; fairness is derived from trip participant rows, which are deleted. Verification: `npm run typecheck` ✅, `npm run build` ✅.

- 2026-04-29 (f): Fixed full-cycle rotation continuity and relative fairness display after browser verification showed all members equal but Today’s driver reset to Administrator and every badge showed `Receiving more`. Root cause: equal drive counts still fell through to under-driven tie sorting, which reset to the first rotation member, and UI displayed raw absolute `drives - rides` scores. Logic correction: driver suggestion now has two phases: unequal drive counts prioritize lowest drive count/lowest fairness/rotation order, while equal drive counts continue from the latest driver’s position in stable rotation order; preview simulates the same sequence. UI fairness now displays group-relative normalized score via `getFairnessScore`, forcing all-equal raw scores to `0`/Balanced while preserving raw counts. Verification: `npm run typecheck` ✅, `npm run build` ✅.

- 2026-04-29 (e): Hardened edit trip driver prefill after browser verification still showed the `Select driver` placeholder. Root cause: edit drawer relied only on `CarPoolTrips.actualDriverId` and Alpine select reconciliation while options are rendered by `x-for`; now the store derives the selected driver from the present driver participant first, falls back to trip `actualDriverId`, normalizes it to a string, and both group/detail edit selects explicitly sync `x-effect` value plus per-option selected state. Verification: `npm run typecheck` ✅, `npm run build` ✅.

- 2026-04-29 (d): Fixed critical Today’s driver suggestion fairness defect found in browser verification. Root cause: client suggestion logic still picked the next member after the latest driver by rotation order, so after Administrator then User 4 drove it wrapped back to Administrator even while User 1/User 2/User 3 had zero drives. Replaced driver suggestion queue in `src/modules/app/carPoolStore.ts` with V1-safe under-driven priority: active members only, lowest drive count first, then lowest fairness score/ride burden, then rotation order/name tie-breaker, avoiding immediate repeat when alternatives exist; upcoming preview now simulates the same queue after each suggested drive. Verification: `npm run typecheck` ✅, `npm run build` ✅.

- 2026-04-29 (c): Fixed follow-up browser verification issues. Root cause for stale Today’s driver was client-side hash-based suggestion logic in `carPoolStore`, which ignored refreshed trip history after create/edit; replaced it with member-order rotation derived from latest logged trip so Today’s driver and upcoming preview recalculate after trip creation/edit and survive page refresh. Fixed edit drawer driver prefill by normalizing stored trip/member IDs to string values matching `AvSelect` option values on group and trip detail drawers. Updated recent trip actions to right-aligned compact `AvButton` + `AvIcon` controls with accessible Edit/View labels and scoped mobile-safe styling. Verification: `npm run typecheck` ✅, `npm run build` ✅.

- 2026-04-29 (b): Completed Astra-assigned Car Pool V1 verification fixes for trip integrity, edit flow, and readability. Added shared server-side validation so driver cannot be submitted as passenger/absent, added protected `updateTrip` action with group membership/context checks and participant rewrite for fairness recalculation, added create/edit drawer mode with driver-filtered passenger/absentee options and immediate driver-selection cleanup, exposed Edit actions from group recent trips and trip detail, loaded group members into trip detail edit state, and scoped light-surface text contrast rules for trip/member/fairness rows and summaries. Verification: no raw app page form controls remain outside Ansiversa components, `npm run typecheck` ✅, `npm run build` ✅.

- 2026-04-29: Aligned Car Pool drawer form controls with Ansiversa component standards by replacing raw text/date inputs, select, textarea, and checkbox inputs in `/app` and group trip/member drawers with `AvInput`, `AvSelect`, `AvTextarea`, and `AvCheckbox`; retained only layout wrapper hooks for day chips/checklists. Verification: `npm run typecheck` ✅, `npm run build` ✅.
...
- 2026-04-27 (b): Seeded remote car-pool Astro DB for `GAL Car Pool` with Jan-Apr 2026 weekday commute history through 2026-04-27 using backend script `scripts/seed-gal-car-pool-2026-jan-apr.ts`. Inserted 82 historical trips plus 328 participant rows, preserved the existing 2026-04-27 trip, and verified final remote totals: Jan 22, Feb 20, Mar 22, Apr 19 trips; 83 participant entries per member. Verification: idempotent remote rerun skipped all 83 existing dates ✅, `npm run typecheck:tsc` ✅.

- 2026-04-27: Fixed unreadable form labels in the standard dark trip/member drawers by adding a drawer-scoped `.app-label` contrast override in `src/styles/global.css`, preserving existing light-card label styling. Verification: `npm run typecheck` ✅, `npm run typecheck:tsc` ✅.
